...
local LOOP_DURATION     = 120   -- seconds before media restart (safety timer)
local LOOP_DELAY_SECS   = 15    -- pause after video ends before restarting
            ...
            if sname == name then
                if active == i and not cooldown[i] then
                    if not waiting[i] then
                        waiting[i] = true
                        log("media_ended: " .. name .. " — restarting in " .. LOOP_DELAY_SECS .. "s")
                        local cb
                        cb = function()
                            obs.timer_remove(cb)
                            waiting[i] = false
                        ...
                        obs.timer_add(cb, LOOP_DELAY_SECS * 1000)
                    end
                end
                return true
            end
